...
class ClassicDurationEngine(Player):
    ...
    def get_card_to_buy(self, money, buys, forced = False, gain_type = "Normal", potions = 0):
        ...
        if self.can_buy("Junk Dealer", money, ignore_debt) and cards_in_deck("Junk Dealer", self) < 1 and get_junk_cards(self) > 4:
            return "Junk Dealer"
            ...
            return "Wharf"
        elif self.can_buy("Gold", money, ignore_debt) and (need_more_village == False or get_pile_size("Fishing Village", supply) == 0):
            return "Gold"
